<template>
  <header class="header container">
    <div class="navbar">
      <router-link class="logo" :to="{name: 'index'}">
        <svg width="91" height="28" viewBox="0 0 91 28" fill="none" xmlns="http://www.w3.org/2000/svg" class="logo-image">
          <g clip-path="url(#clip0)">
          <path d="M26.9947 6.65895C26.9632 6.36854 26.7827 6.11151 26.513 5.98563L13.8567 0.0793852C13.6306 -0.0261362 13.3693 -0.0261362 13.1431 0.0793852L0.486896 5.98563C0.217213 6.11151 0.0367559 6.36849 0.00516797 6.65895C0.00448242 6.66501 0 6.7458 0 6.75023V21.094C0 21.439 0.210041 21.7493 0.530402 21.8773L13.1867 26.9398C13.2872 26.9801 13.3936 27.0002 13.5 27.0002C13.6064 27.0002 13.7128 26.9801 13.8133 26.9398L26.4696 21.8773C26.79 21.7493 27 21.439 27 21.094V6.75023C27 6.7458 26.9954 6.66496 26.9947 6.65895ZM13.5 1.77506L24.0334 6.69064L19.9774 8.31307L9.24576 3.7604L13.5 1.77506ZM7.18822 4.72059L17.7533 9.20264L13.5 10.904L2.96662 6.69064L7.18822 4.72059ZM1.6875 7.99645L12.6562 12.3839V24.9102L1.6875 20.5227V7.99645ZM14.3438 24.9103V12.3839L25.3125 7.99645V20.5227L14.3438 24.9103Z" fill="#0A143A"/></g>
          <path d="M44.1562 18.4844H37.9922L36.8203 22H33.082L39.4336 4.9375H42.6914L49.0781 22H45.3398L44.1562 18.4844ZM38.9414 15.6367H43.207L41.0625 9.25L38.9414 15.6367ZM61.875 15.7773C61.875 17.8086 61.4414 19.3945 60.5742 20.5352C59.707 21.668 58.4961 22.2344 56.9414 22.2344C55.5664 22.2344 54.4688 21.707 53.6484 20.6523L53.4961 22H50.4492V4H53.8359V10.457C54.6172 9.54297 55.6445 9.08594 56.918 9.08594C58.4648 9.08594 59.6758 9.65625 60.5508 10.7969C61.4336 11.9297 61.875 13.5273 61.875 15.5898V15.7773ZM58.4883 15.5312C58.4883 14.25 58.2852 13.3164 57.8789 12.7305C57.4727 12.1367 56.8672 11.8398 56.0625 11.8398C54.9844 11.8398 54.2422 12.2812 53.8359 13.1641V18.168C54.25 19.0586 55 19.5039 56.0859 19.5039C57.1797 19.5039 57.8984 18.9648 58.2422 17.8867C58.4062 17.3711 58.4883 16.5859 58.4883 15.5312ZM67.5469 22H64.1484V9.32031H67.5469V22ZM63.9492 6.03906C63.9492 5.53125 64.1172 5.11328 64.4531 4.78516C64.7969 4.45703 65.2617 4.29297 65.8477 4.29297C66.4258 4.29297 66.8867 4.45703 67.2305 4.78516C67.5742 5.11328 67.7461 5.53125 67.7461 6.03906C67.7461 6.55469 67.5703 6.97656 67.2188 7.30469C66.875 7.63281 66.418 7.79688 65.8477 7.79688C65.2773 7.79688 64.8164 7.63281 64.4648 7.30469C64.1211 6.97656 63.9492 6.55469 63.9492 6.03906ZM74.4023 6.20312V9.32031H76.5703V11.8047H74.4023V18.1328C74.4023 18.6016 74.4922 18.9375 74.6719 19.1406C74.8516 19.3438 75.1953 19.4453 75.7031 19.4453C76.0781 19.4453 76.4102 19.418 76.6992 19.3633V21.9297C76.0352 22.1328 75.3516 22.2344 74.6484 22.2344C72.2734 22.2344 71.0625 21.0352 71.0156 18.6367V11.8047H69.1641V9.32031H71.0156V6.20312H74.4023ZM77.5781 15.543C77.5781 14.2852 77.8203 13.1641 78.3047 12.1797C78.7891 11.1953 79.4844 10.4336 80.3906 9.89453C81.3047 9.35547 82.3633 9.08594 83.5664 9.08594C85.2773 9.08594 86.6719 9.60938 87.75 10.6562C88.8359 11.7031 89.4414 13.125 89.5664 14.9219L89.5898 15.7891C89.5898 17.7344 89.0469 19.2969 87.9609 20.4766C86.875 21.6484 85.418 22.2344 83.5898 22.2344C81.7617 22.2344 80.3008 21.6484 79.207 20.4766C78.1211 19.3047 77.5781 17.7109 77.5781 15.6953V15.543ZM80.9648 15.7891C80.9648 16.9922 81.1914 17.9141 81.6445 18.5547C82.0977 19.1875 82.7461 19.5039 83.5898 19.5039C84.4102 19.5039 85.0508 19.1914 85.5117 18.5664C85.9727 17.9336 86.2031 16.9258 86.2031 15.543C86.2031 14.3633 85.9727 13.4492 85.5117 12.8008C85.0508 12.1523 84.4023 11.8281 83.5664 11.8281C82.7383 11.8281 82.0977 12.1523 81.6445 12.8008C81.1914 13.4414 80.9648 14.4375 80.9648 15.7891Z" fill="#0A143A"/>
          <defs><clipPath id="clip0"><rect width="27" height="27" fill="white" /></clipPath></defs>
        </svg>
      </router-link>

      <div class="navbar-panel" :class="isMenuOpen ? 'is-open': ''">
        <button class="close-menu" @click="isMenuOpen = false">
          <svg fill="#000000" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24px" height="24px">
            <path d="M 4.7070312 3.2929688 L 3.2929688 4.7070312 L 10.585938 12 L 3.2929688 19.292969 L 4.7070312 20.707031 L 12 13.414062 L 19.292969 20.707031 L 20.707031 19.292969 L 13.414062 12 L 20.707031 4.7070312 L 19.292969 3.2929688 L 12 10.585938 L 4.7070312 3.2929688 z"/>
          </svg>
        </button>
        <nav class="navbar-menu">
          <ul class="menu">
            <li
              class="menu-item"
              v-for="category in categories"
              :key="category.id"
            >
              <router-link
                class="menu-link"
                :to="{name: 'filteredAdsWithCategory', params: {category: category.slug}}"
                >{{category.title}}
              </router-link>
            </li>
          </ul>
        </nav>

        <div class="header_button-group">
          <img
            v-if="userToken"
            :src="user.image_url"
            class="header-avatar"
            @click="areDetailsVisible = !areDetailsVisible"
          />
          <a class="button button-link" @click="openAuthModal" v-else>Login and registration</a>

          <div class="details" v-if="areDetailsVisible">
            <ul>
              <li
                v-for="route in routes"
                :key="route.name"
              >
                <router-link
                  :to="{name: route.name}"
                  class="details-button button-link"
                  @click="[areDetailsVisible, isMenuOpen] = [false, false]"
                  >{{route.title}}
                </router-link>
              </li>

              <li @click="logout">
                <span class="details-button logout-button button-link">Logout</span>
              </li>
            </ul>
          </div>

          <router-link
            class="button button-primary"
            :to="{name: 'create'}"
            @click="isMenuOpen = false"
            >Post an ad
          </router-link>
        </div>
      </div>

      <!-- burger menu button -->
      <button class="menu-button" @click="isMenuOpen = true">
        <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg" >
          <path d="M0.937476 9.37501H29.0624C29.58 9.37501 29.9999 8.95498 29.9999 8.43754C29.9999 7.92002 29.5799 7.50006 29.0624 7.50006H0.937476C0.419955 7.50006 0 7.92009 0 8.43754C0 8.95498 0.420033 9.37501 0.937476 9.37501Z" fill="black"/>
          <path d="M29.0624 14.0626H0.937476C0.419955 14.0626 0 14.4826 0 15C0 15.5175 0.420033 15.9375 0.937476 15.9375H29.0624C29.58 15.9375 29.9999 15.5175 29.9999 15C29.9999 14.4826 29.58 14.0626 29.0624 14.0626Z" fill="black"/>
          <path d="M29.0625 20.625H10.3125C9.79495 20.625 9.375 21.0451 9.375 21.5625C9.375 22.08 9.79503 22.5 10.3125 22.5H29.0625C29.58 22.5 29.9999 22.0799 29.9999 21.5625C30 21.045 29.58 20.625 29.0625 20.625Z" fill="black"/>
        </svg>
      </button>
    </div>
  </header>
  
  <router-view></router-view>

  <AuthModal
    v-if="showAuthModal"
    @closeAuthModal="closeAuthModal"
  />
</template>

<script setup>
import AuthModal from './AuthModal.vue'
import { useStore } from 'vuex'
import { useRouter } from 'vue-router'
import { ref, computed } from 'vue'

const store = useStore()
const router = useRouter()

const userToken = computed(() => store.state.user.token)
const categories = computed(() => store.state.categories)

store.dispatch('getCategories')

const showAuthModal = ref(false)
const isMenuOpen = ref(false)
const areDetailsVisible = ref(false)

const user = ref({
  name: '',
  image_url: '',
})

const routes = [
  {
    name: 'editProfile',
    title: 'Profile settings'
  },
  {
    name: 'favorites',
    title: 'Favorites'
  },
  {
    name: 'myAds',
    title: 'My ads'
  },
]

const openAuthModal = () => showAuthModal.value = true
const closeAuthModal = () => showAuthModal.value = false

const logout = () => {
  areDetailsVisible.value = false
  isMenuOpen.value = false
  if ( confirm('Are you sure you want to logout?') ) {
    store.dispatch('logout').then(router.push('/'))
  }
}

store.dispatch('getAuthUser')
  .then(data => {
    user.value.name = data.name
    user.value.image_url = data.image_url
  })
  .catch(err => {
    console.log('Unauthenticated');
  })

</script>

<style scoped>
.details {
  position: absolute;
  display: block;
  top: 75px;
  z-index: 5;
  text-align: center;
  background: #fff;
  border: 1px solid #0A143A;
  border-radius: 6px;
}
.details ul {
  margin: 0;
  padding-left: 0;
}
.details ul li {
  list-style: none;
}
.details-button {
  text-decoration: none;
  display: inline-block;
  cursor: pointer;
  padding: 5px 10px;
}
.logout-button {
  color: red;
}
</style>